"""Local Ollama-powered AI helpers for StaffAlloc."""
...
class OllamaConfigurationError(RuntimeError):
    """Raised when the Ollama service cannot be configured or reached."""


class OllamaInvocationError(RuntimeError):
...
def _call_ollama(
    ...
    *,
    ...
    """Call the local Ollama API with retry logic."""
    max_retries = 1
    retry_count = 0
    ...
    while retry_count <= max_retries:
        try:
            logger.info(
                f"Calling Ollama API: model={settings.LLM_MODEL_NAME}, "
                f"max_tokens={max_output_tokens}, attempt={retry_count+1}"
            )
            
            # Ollama API expects POST to /api/generate
            ...
            raise OllamaInvocationError("Ollama did not return a 'response' field.")
            
        except httpx.ConnectError as exc:
            raise OllamaConfigurationError(
                f"Cannot connect to Ollama at {settings.OLLAMA_API_URL}. "
            ...
            error_msg = str(exc)
            logger.error(
                f"Ollama API call failed: {error_msg} (attempt {retry_count}/{max_retries+1})"
            )
            
            # Check if it's a model not found error
            if exc.response.status_code == 404:
                raise OllamaConfigurationError(
                    f"Model '{settings.LLM_MODEL_NAME}' not found. "
                    f"Please run: ollama pull {settings.LLM_MODEL_NAME}"
                ) from exc
            
            # If we've exhausted retries, raise the error
            if retry_count > max_retries:
                raise OllamaInvocationError(
                    f"Ollama request failed: {error_msg}. Try a simpler question or wait a moment."
                ) from exc
            ...
            retry_count += 1
            logger.error(
                f"Ollama API call failed: {exc} (attempt {retry_count}/{max_retries+1})"
            )
            
            if retry_count > max_retries:
                raise OllamaInvocationError(
                    f"Ollama request failed: {exc!s}. "
    ...
    raise OllamaInvocationError("Ollama did not return any text output.")
...
def suggest_header_mapping(
    *, headers: Sequence[str], required_fields: Sequence[str], sheet_name: str
    ...
    """Use local LLM to map spreadsheet headers to expected field names."""
    ...
    response_text = _call_ollama(prompt, temperature=0.1, max_output_tokens=256)
    ...
    try:
        ...
        raise OllamaInvocationError(
            "LLM returned an invalid header mapping JSON."
        ) from exc

    if not isinstance(mapping, dict):
        raise OllamaInvocationError(
            "LLM returned an unexpected header mapping structure."
        )
...
def generate_chat_response(
    ...
    *,
    ...
    answer = _call_ollama(prompt, max_output_tokens=1024, temperature=0.3)
...
def scan_allocation_conflicts(db: Session, *, manager_id: Optional[int] = None) -> Tuple[List[Dict[str, object]], str]:
    ...
    try:
        ...
        prompt = "\n".join(prompt_lines) + "\n\nMitigation guidance:"
        reasoning = _call_ollama(prompt, temperature=0.2)
        message += reasoning
    except (OllamaConfigurationError, OllamaInvocationError):
...
def generate_forecast_insights(db: Session, *, months_ahead: int = 3, manager_id: Optional[int] = None) -> Tuple[List[Dict[str, object]], str]:
    ...
    try:
        ...
        reasoning = _call_ollama(prompt, temperature=0.3)
        message += reasoning
    except (OllamaConfigurationError, OllamaInvocationError):
        # Provide basic guidance without AI
        if shortages:
