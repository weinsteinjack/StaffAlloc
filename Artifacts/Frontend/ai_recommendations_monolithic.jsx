...
type Recommendation = {
  category: string;
  categoryColor: 'blue' | 'red' | 'purple';
  priority: Priority;
  rightTag: string;
  rightTagColor: 'red' | 'yellow';
  title: string;
  description: string;
...
const recommendationsData: Recommendation[] = [
  ...
  },
  {
    ...
    categoryColor: 'red',
    priority: 'MEDIUM',
    rightTag: '', // This one is empty in the design
    rightTagColor: 'yellow', // Placeholder
  ...
  {
    category: 'Training', // Implied category
    categoryColor: 'blue', // Implied color
    priority: 'LOW',
    rightTag: '', // Empty
    rightTagColor: 'yellow', // Placeholder
...
// Reusable PillTag component
const PillTag: FC<{ text: string; color: string; type: 'priority' | 'category' }> = ({ text, color, type }) => {
  const colorClasses = {
    priority: {
  ...
  const classes = type === 'priority' ? colorClasses.priority[text.split(' ')[0] as Priority] : colorClasses.category[color as keyof typeof colorClasses.category];
...
const DonutChart: FC<{ percentage: number; size?: number; strokeWidth?: number }> = ({ percentage, size=160, strokeWidth=18 }) => {
      ...
      <header className="bg-gradient-to-r from-indigo-600 to-purple-700 text-white shadow-md">
        <nav className="max-w-screen-xl mx-auto px-6 py-3 flex justify-between items-center">
          ...
          <ul className="hidden md:flex items-center gap-2">
            {['Dashboard', 'Projects', 'Staff', 'Reports'].map(item => (
              <li key={item}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 flex flex-col gap-6">
            {recommendationsData.map((rec, index) => (
              <div key={index} className="bg-white p-6 rounded-xl shadow-md border border-slate-200/50 hover:shadow-lg transition-shadow">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-2">
                    <PillTag text={rec.category} color={rec.categoryColor} type="category" />
                    {rec.priority && <PillTag text={`${rec.priority} PRIORITY`} color={rec.priority} type="priority" />}
                  </div>
                  {rec.rightTag && <PillTag text={rec.rightTag} color={rec.rightTagColor} type="category" />}
                </div>
          ...
          <aside className="lg:col-span-1">
            <div className="bg-white p-6 rounded-xl shadow-md border border-slate-200/50 sticky top-8">
              ...
              <div className="space-y-6">
                <div>
                  <h3 className="font-semibold text-slate-800 border-b pb-2 mb-3">Upcoming Capacity Gaps</h3>
                ...
                <div>
                  <h3 className="font-semibold text-slate-800 border-b pb-2 mb-3">Top Skills in Demand</h3>
                  <div className="text-sm text-slate-600 space-y-1">
                    {topSkills.map((skill, i) => <p key={i}>{skill}</p>)}
                  </div>
                </div>
