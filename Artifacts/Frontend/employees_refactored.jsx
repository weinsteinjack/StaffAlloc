...
const employeeData: Employee[] = [
  ...
  { id: 8, name: 'SQL', title: 'Bettentont@eproia', email: '@mail', avatarUrl: 'https://images.unsplash.com/photo-1517841905240-472988babdf9?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80', initials: 'AUG', initialsBgColor: 'bg-blue-500', skills: [], status: { type: 'multiple', value: '45% allocated', secondaryValue: '' } },
  { id: 9, name: 'Fnoc Fuipter', title: 'Setsksnrt & tnqtckter', email: '@mail', avatarUrl: 'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80', initials: 'JSE', initialsBgColor: 'bg-green-500', skills: [{ name: 'Eaqceoi', color: 'blue' }, { name: 'Pipass', color: 'indigo' }], status: { type: 'multiple', value: '45% allocated', secondaryValue: 'On Leave' } },
  { id: 10, name: 'Las Finger', title: 'Rossinrak & equlitrelis', email: '@mail', avatarUrl: 'https://images.unsplash.com/photo-1522075469751-3a6694fb2f61?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80', initials: 'JWS', initialsBgColor: 'bg-blue-500', skills: [{ name: 'Figma', color: 'purple' }, { name: 'Sceintre', color: 'orange' }], status: { type: 'multiple', value: '45% allocated', secondaryValue: 'On Leave' } },
  { id: 11, name: 'AWS', title: 'Riecttvnf@bssimeer', email: '@mail', avatarUrl: 'https://images.unsplash.com/photo-1530268729831-4b0b9e170218?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80', initials: 'SIS', initialsBgColor: 'bg-red-500', skills: [{ name: 'AWS', color: 'blue' }, { name: 'AWS', color: 'blue' }], status: { type: 'multiple', value: '45% allocated', secondaryValue: 'gap 6' } },
  { id: 12, name: 'Florlundv', title: 'Rottwart@eositpiter', email: '@mail', avatarUrl: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80', initials: 'AJE', initialsBgColor: 'bg-orange-500', skills: [], status: { type: 'multiple', value: '06% allocated', secondaryValue: 'On Leave' } },
];
  ...
  return (
    <svg viewBox="0 0 64 64" className="w-32 h-32 transform -rotate-90">
      {data.map((item, index) => {
        ...
        return (
          <circle key={index} r="28" cx="32" cy="32" fill="transparent" stroke={item.color}
            strokeWidth="8" strokeDasharray={strokeDasharray} strokeDashoffset={strokeDashoffset} />
        );
      })}
    </svg>
  );
...
const AppHeader: React.FC<{ navLinks: string[] }> = ({ navLinks }) => (
    ...
    <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="flex justify-between items-center h-16">
        ...
        <nav className="hidden md:flex md:gap-8">
          {navLinks.map((link) => (
            <a key={link} href="#" className={`text-sm font-medium ${link === 'Dashboard' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-slate-500 hover:text-slate-700'} py-5`}>
              {link}
            </a>
          ))}
        </nav>
...
const TeamMembersHeader: React.FC = () => (
...
const FilterBar: React.FC<{ filterPills: string[] }> = ({ filterPills }) => (
  <div className="flex flex-col sm:flex-row items-center gap-4">
    <div className="relative w-full sm:w-auto sm:flex-grow">
      <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={20} />
      <input type="text" placeholder="Search employees..." className="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
    </div>
    <div className="flex items-center gap-2 flex-wrap">
      {filterPills.map((pill) => (
        <button key={pill} className={`px-3 py-1.5 text-sm font-medium rounded-lg border ${pill === 'All' ? 'bg-blue-500 text-white border-blue-500' : 'bg-white text-slate-700 border-slate-300 hover:bg-slate-50'}`}>
          {pill}
        </button>
      ))}
    </div>
...
const EmployeeCard: React.FC<{ employee: Employee }> = ({ employee }) => (
  <div className="bg-white rounded-xl shadow-md border border-slate-200/75 p-4 flex flex-col items-center text-center transition-shadow hover:shadow-lg">
    ...
    <div className="flex flex-wrap justify-center gap-2 mb-4 h-5">
      {employee.skills.map((skill, index) => <SkillTag key={index} skill={skill} />)}
    </div>
...
const AdditionalFiltersWidget: React.FC = () => (
  <SidebarCard>
  ...
  <SidebarCard>
    ...
    <div className="flex items-center gap-6">
      ...
      <div className="space-y-2 text-sm">
        <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-blue-500"></div><span className="text-slate-600">47%</span></div>
        <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-green-200"></div><span className="text-slate-600">38%</span></div>
        <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-gray-300"></div><span className="text-slate-600">15%</span></div>
      </div>
    </div>
  </SidebarCard>
);
...
const AvailabilityCalendarWidget: React.FC = () => {
      ...
      <main className="max-w-screen-2xl mx-auto p-4 sm:p-6 lg:p-8">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_384px] gap-8">
          ...
          <section className="flex flex-col gap-6">
            <TeamMembersHeader />
            <FilterBar filterPills={filterPills} />
            <EmployeeGrid employees={employeeData} />
          </section>
          
          {/* Right Column: Sidebar */}
          <aside className="space-y-6">
            <AdditionalFiltersWidget />
            <TeamOverviewWidget />
            <TopSkillsWidget />
