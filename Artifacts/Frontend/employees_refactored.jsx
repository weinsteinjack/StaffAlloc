...
const StatusIndicator = ({ status }: StatusIndicatorProps) => (
  <div className="flex items-center gap-2">
    <div className={`w-2.5 h-2.5 rounded-full ${status === 'Available' ? 'bg-green-500' : 'bg-green-500'}`}></div>
...
const ProgressBar = ({ progress }: ProgressBarProps) => (
  <>
    ...
    <div className="flex justify-end">
        <span className="text-slate-800 font-medium">{progress > 100 ? `${(progress / 10).toFixed(1)}%` : `${progress}%`}</span>
    </div>
  </>
);
...
const EmployeeCard = ({ employee }: { employee: Employee }) => (
  <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-6 flex flex-col hover:shadow-md transition-shadow duration-200">
    <div className="flex items-center gap-4">
      <Avatar avatar={employee.avatar} isInitials={employee.isInitials} name={employee.name} />
      <div>
